//
//  RepeatMonthIntervalViewController.swift
//  Milu
//
...
class RepeatMonthIntervalViewController: UIViewController,UITableViewDelegate,UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if indexPath.row == self.monthInterval-1 {
            intervalCell.checkImageView.isHidden = false
        }else{
            intervalCell.checkImageView.isHidden = true
        }
        if indexPath.row > 0 {
            intervalCell.intervalItemLabel.text = "\(indexPath.row + 1) months"
        }else{
            intervalCell.intervalItemLabel.text = "\(indexPath.row + 1) month"
        }
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        self.monthInterval = indexPath.row + 1
