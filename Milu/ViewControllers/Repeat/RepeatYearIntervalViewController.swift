//
//  RepeatYearIntervalViewController.swift
//  Milu
//
...
class RepeatYearIntervalViewController: UIViewController,UITableViewDelegate,UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if indexPath.row == self.yearInterval-1 {
            intervalCell.checkImageView.isHidden = false
        }else{
            intervalCell.checkImageView.isHidden = true
        }
        if indexPath.row > 0 {
            intervalCell.intervalItemLabel.text = "\(indexPath.row + 1) years"
        }else{
            intervalCell.intervalItemLabel.text = "\(indexPath.row + 1) year"
        }
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        self.yearInterval = indexPath.row + 1
