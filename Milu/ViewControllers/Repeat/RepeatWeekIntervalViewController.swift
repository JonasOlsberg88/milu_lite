//
//  RepeatWeekIntervalViewController.swift
//  Milu
//
...
class RepeatWeekIntervalViewController: UIViewController,UITableViewDelegate,UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if indexPath.row == self.weekInterval-1 {
            intervalCell.checkImageView.isHidden = false
        }else{
            intervalCell.checkImageView.isHidden = true
        }
        if indexPath.row > 0 {
            intervalCell.intervalItemLabel.text = "\(indexPath.row + 1) weeks"
        }else{
            intervalCell.intervalItemLabel.text = "\(indexPath.row + 1) week"
        }
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        self.weekInterval = indexPath.row + 1
